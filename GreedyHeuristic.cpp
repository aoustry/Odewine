...
GreedyHeuristic::GreedyHeuristic( IloNumArray la, SceneInstance SI_input, vector<int> tabu){
	SI = SI_input;
	n = SI.n;
	m = SI.m;
	gamma_self = SI.gamma;
	...
	for (int i = 0; i<n ;i++){
		objective.push_back(la[i] - SI.lambda*(SI.d[i]+SI.u[i]));
	}


	for (int j =0 ; j<m; j++){
		objective.push_back(la[n+j] + SI.c[j]);
		ap_pot.push_back(j);
		y_on.push_back({});
...
vector<int> GreedyHeuristic::selection(vector<int> tabu_iter){
...
void GreedyHeuristic::update(vector <int> selected_ones){
...
bool GreedyHeuristic::check_possible(int k, bool is_ap, Matrix MU , Matrix MD ){
...
vector <vector <float> > GreedyHeuristic::initM(Matrix B, int AP, bool down){
...
bool GreedyHeuristic::compute(IloNumArray true_l_star, float mu_star,IloBoolArray pattern, IloNumArray cp_tab, vector<tuple<int, int> > &edges){
	...
	//Reinit reduced_cost cp_tab, pattern

		float reduced_cost = mu_star ;
		cp_tab[0]=0;
		for (int i = 0; i<n+m; i++){
			pattern[i]=0;
		}
		...
		for (Iter it2 = ap_on.begin(); it2!= ap_on.end(); it2++){
			...
			pattern[j+n]=1;
			cp_tab[0]+=SI.c[j];
			reduced_cost+= true_l_star[n+j] + SI.c[j];
			vector<int> connected_clients = y_on[j];
			for (Iter it = connected_clients.begin(); it!= connected_clients.end(); it++){
				i = *it;
				pattern[i] = 1;
				reduced_cost+= true_l_star[i] - SI.lambda*(SI.d[i]+SI.u[i]);
				cp_tab[0]+=-SI.lambda*(SI.u[i]+SI.d[i]);
		...
		//cout <<"Reduced cost ";
		//cout << reduced_cost << "\n";
		return reduced_cost<-2;

	// Si ap_on est vide, pattern est un tableau de 0 et cp[0] vaut 0 : ok
}

bool GreedyHeuristic::check_validity_solution(){
	...
	for (Iter it2 = ap_on.begin(); it2!= ap_on.end(); it2++){
		...
		for (Iter it = connected_clients.begin(); it!= connected_clients.end(); it++){
			i = *it;
			load+=SI.d[i] + SI.u[i];
			noiseU = SI.sigma ; noiseD = SI.sigma ;

			//Calcul noise
			for (Iter it3 = ap_on.begin(); it3!= ap_on.end(); it3++){
				  k = *it3;
					if (k!=j){noiseU+=SI.p[k+n][j+n]; noiseD+=SI.p[k+n][i];}
			}
			for (Iter it3 = cl_on.begin();it3!=cl_on.end();it3++){
				k = *it3;
				if (connected(k,j) == false){noiseU+=SI.p[k][j+n]; noiseD+=SI.p[k][i];}
			}

			if (noiseU>(SI.p[i][j+n]/(pow(2,SI.u[i]) -1))){return false;}
			if (noiseD >(SI.p[j+n][i]/(pow(2,SI.d[i]) -1))){return false;}
		}
		if (load>gamma_self){return false;}
	}
...
bool GreedyHeuristic::connected(int client, int ap){
	int i;
	vector<int> connected_clients = y_on[ap];
